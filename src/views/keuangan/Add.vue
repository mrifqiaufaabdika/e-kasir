    ...
    >
      <v-row>
        ...
        >
          ...
          >
            <div>
              <v-select v-model="datas.bisnis" label="Pilih Bisnis" :items="items.bisnis" outlined :rules="[rules.required]" />
              <v-select v-model="datas.jenis" label="Jenis" :items="items.jenis" outlined :rules="[rules.required]" />
              <v-textarea v-model="datas.keterangan" label="Keterangan" outlined  :rules="[rules.required]"/>
              <v-text-field v-model="datas.total" label="Total" outlined :rules="[rules.required]" type="number" />
              <v-btn
                      color="green"
                      large
                      :dark="!dataValidation"
...
export default {
  ...
  created () {
    this.getKeuanganCreate().then(data => {
      this.items.bisnis = isEmpty(data.bisnis, {})
    })

    this.datas.pencatat = this.user.nip
  },
  methods: {
    ...mapActions(['getKeuanganCreate', 'addKeuangan']),
    backButton () {
      this.$router.push({ name: 'keuangan_masuk' })
    },
    postSave () {
      ...
      this.addKeuangan(formData).then((res) => {
