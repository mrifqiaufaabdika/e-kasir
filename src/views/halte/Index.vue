...
export default {
  ...
  methods: {
    ...mapActions(['getHalte', 'deleteHalte']),
    can,
    _detail (value) {
      this.$router.push({ name: 'halte_view', params: { id: value.id } })
    },
    _add () {
      this.$router.push({ name: 'halte_add' })
    },
    _edit (value) {
      this.$router.push({ name: 'halte_edit', params: { id: value.id } })
    },
    _delete (value) {
      if (value === true) {
        ...
        this.dcMessages = 'Sedang Menghapus Halte'
        this.deleteHalte(this.deleteId).then(res => {
          this._loadData(true)
        ...
        this.dcMessages = `Hapus Keuangan <span class="pink--text">#${this.deleteId}</span> ?`
        this.showDC = true
      }
    },
    _clearFilter () {
